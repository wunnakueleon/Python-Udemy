from tkinter import *
import math


# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
PURPLE = "#B7BDF7"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
streak = ""
timer = None

# ---------------------------- TIMER RESET ------------------------------- # 
def reset():
    global reps
    global streak 

    reps = 0
    streak = ""
    timer_label.config(text="Timer")
    check_mark.config(text=streak)
    canvas.itemconfig(timer_text, text="00:00")
    window.after_cancel(timer)

    
    
# ---------------------------- TIMER MECHANISM ------------------------------- # 

def start_timer():
    global reps
    global streak

    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN * 60
    long_break_sec = LONG_BREAK_MIN * 60

    
    reps += 1

    if reps % 2 != 0:
        streak += "✅"
        count_down(work_sec)
        timer_label.config(text="Work")
        check_mark.config(text=streak)

    elif reps % 2 == 0 and reps != 8:
        timer_label.config(text="Short Break", fg=RED)
        count_down(short_break_sec)
    else:
        timer_label.config(text="Long Break")
        count_down(long_break_sec)

# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 

def count_down(count):
    global timer
    
    minute = math.floor(count / 60)
    second = count % 60
    if second < 10:
        second = "0" + str(second)

    canvas.itemconfig(timer_text, text=f"{minute}:{second}")
    if count > 0:
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()


# ---------------------------- UI SETUP ------------------------------- #


window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=PURPLE)

timer_label = Label(text="Timer", font=(FONT_NAME, 38), bg=PURPLE, fg=GREEN)
timer_label.grid(column=1, row=0)

canvas = Canvas(width=200, height=224, bg=PURPLE, highlightthickness=0)
tomato_img = PhotoImage(file="pomodoro-start/tomato.png")
canvas.create_image(100, 112, image=tomato_img)
timer_text = canvas.create_text(103, 121, text="00:00", fill="white", font=(FONT_NAME, 25, "bold"))
canvas.grid(column=1, row=1)

start_button = Button(text="start", highlightthickness=0, command=start_timer)
start_button.grid(column=0, row=2)

reset_button = Button(text="reset", highlightthickness=0, command=reset)
reset_button.grid(column=2, row=2)

check_mark = Label(text=streak, fg=PURPLE, bg=PURPLE)
check_mark.grid(column=1, row=3)

window.mainloop()